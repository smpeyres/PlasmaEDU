...
# Function to calculate the total kinetic energy of the particles
def calculate_kinetic_energy(Vx, Vy, Vz, m, Np):
     kinetic_energy = np.zeros(Vx.shape[0])
     ...
     # Convert kinetic energy from Joules to electron volts (eV)
     kinetic_energy_eV = kinetic_energy / np.abs(qe)

     time_averaged_kinetic_energy_eV = np.mean(kinetic_energy_eV)
     print(f'Time-averaged kinetic energy: {time_averaged_kinetic_energy_eV:.2e} eV')
     return time_averaged_kinetic_energy_eV
