...
def inf_helix_Tominaka(Ra,La,I0, phi0, x, y, z):
    ...
    # Eq 8
    Az_inside = -(mu0*I0/(2*np.pi))*np.log(Ra) + (mu0*I0/np.pi)*(special.kv(1, 1*k*Ra)*special.iv(1, 1*k*r)*np.cos(1*(phi - phi0 - k*z))
                                                                 + special.kv(2, 2*k*Ra)*special.iv(2, 2*k*r)*np.cos(2*(phi - phi0 - k*z)))
    
    # Eq 9
    Ar_inside = -(mu0*I0/(2*np.pi))*k*Ra *( (special.kv(2, 1*k*Ra)*special.iv(2, 1*k*r) - special.kv(0, 1*k*Ra)*special.iv(0, 1*k*r))*np.sin(1*(phi - phi0 - k*z)) 
                                          + (special.kv(3, 2*k*Ra)*special.iv(3, 2*k*r) - special.kv(1, 2*k*Ra)*special.iv(1, 2*k*r))*np.sin(2*(phi - phi0 - k*z)) )
    
    # Eq 10
    Aphi_inside = (mu0*I0/(4*np.pi))*k*r + (mu0*I0/(2*np.pi))*k*Ra *( (special.kv(2, 1*k*Ra)*special.iv(2, 1*k*r) + special.kv(0, 1*k*Ra)*special.iv(0, 1*k*r))*np.sin(1*(phi - phi0 - k*z)) 
                                          + (special.kv(3, 2*k*Ra)*special.iv(3, 2*k*r) + special.kv(1, 2*k*Ra)*special.iv(1, 2*k*r))*np.sin(2*(phi - phi0 - k*z)) )
    
    # Eq 11
    Az_outside = -(mu0*I0/(2*np.pi))*np.log(r) + (mu0*I0/np.pi)*(special.iv(1, 1*k*Ra)*special.kv(1, 1*k*r)*np.cos(1*(phi - phi0 - k*z))
                                                                 + special.iv(2, 2*k*Ra)*special.kv(2, 2*k*r)*np.cos(2*(phi - phi0 - k*z)))
    
    # Eq 12
    Ar_outside = -(mu0*I0/(2*np.pi))*k*Ra *( (special.iv(2, 1*k*Ra)*special.kv(2, 1*k*r) - special.iv(0, 1*k*Ra)*special.kv(0, 1*k*r))*np.sin(1*(phi - phi0 - k*z)) 
                                          + (special.ki(3, 2*k*Ra)*special.kv(3, 2*k*r) - special.iv(1, 2*k*Ra)*special.kv(1, 2*k*r))*np.sin(2*(phi - phi0 - k*z)) )
    
    # Eq 13
    Aphi_outside = (mu0*I0/(4*np.pi))*k*Ra*Ra/r + (mu0*I0/(2*np.pi))*k*Ra *( (special.iv(2, 1*k*Ra)*special.kv(2, 1*k*r) + special.iv(0, 1*k*Ra)*special.kv(0, 1*k*r))*np.sin(1*(phi - phi0 - k*z)) 
                                          + (special.iv(3, 2*k*Ra)*special.kv(3, 2*k*r) + special.iv(1, 2*k*Ra)*special.kv(1, 2*k*r))*np.sin(2*(phi - phi0 - k*z)) )
