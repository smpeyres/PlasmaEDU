...
def helix(Ra, La, N, I0, Point, Nsegments=100):
    """
    Computes the magnetic field at a given point generated by a helical wire using the Biot-Savart law.

    Parameters:
    - Ra        : Loop radius [m]
    - La        : Axial separation between turns [m]
    - N         : Number of turns in the helix
    - I0        : Current in the wire [A]
    - Point     : (X, Y, Z) The point at which to calculate the magnetic field [m]
    - Nsegments : Number of discrete segments for the filament

    Returns:
    - Bx, By, Bz : Cartesian components of the magnetic field at the point
    """
    ...
    # Initialize total magnetic field components
    Bx_total, By_total, Bz_total = 0.0, 0.0, 0.0

    # Discretize the helical coil into small elements
    theta_vals = np.linspace(0, 2 * np.pi * N, Nsegments)

    for i in range(1, len(theta_vals)):
        # Position of the current element (filament) at theta(i) and theta(i-1)
        theta = theta_vals[i]
        theta_prev = theta_vals[i - 1]

        # Position of the filament at theta and theta_prev
        r_filament = np.array([Ra * np.cos(theta), Ra * np.sin(theta), La * theta / (2 * np.pi)])
        r_filament_prev = np.array([Ra * np.cos(theta_prev), Ra * np.sin(theta_prev), La * theta_prev / (2 * np.pi)])

        # Current element dL (from r_filament_prev to r_filament)
        dl = r_filament - r_filament_prev

        # Position vector from filament element to the observation point
        r_vec = Point - r_filament

        # Distance from filament element to observation point
        r_mag = np.linalg.norm(r_vec)

        if r_mag == 0:
            continue  # Avoid singularity when the point is exactly on the filament

        # Unit vector in the direction of r_vec
        r_hat = r_vec / r_mag

        # Compute the cross product dL x r_hat
        dl_cross_rhat = np.cross(dl, r_hat)

        # Compute the Biot-Savart contribution to the magnetic field at this point
        dB = (mu0 * I0 / (4 * np.pi * r_mag**2)) * dl_cross_rhat
    ...
    # Return the total magnetic field components at the observation point
